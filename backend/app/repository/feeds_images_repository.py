...
class FeedsImagesRepository:
    @staticmethod
    def get_model_one_data(session, model:str, model_id: int, sort_order: int = None):
        query = session.query(FeedsImages).filter(
            FeedsImages.img_model == model,
            FeedsImages.img_model_id == model_id
        )

        if sort_order is not None:
            query = query.filter(FeedsImages.sort_order == sort_order)
        return query.order_by(FeedsImages.sort_order.asc()).first()
    ...
    async def upload(session, model_id: int, file, ext: str, path: str = "feeds", sort_order: int = 0):
        ...
        try:
            ...
            return FeedsImagesRepository.create(session, image_params, is_commit=False)
