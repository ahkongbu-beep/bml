...
class CommunitiesComments(Base):
    ...
    community_id = Column(Integer, nullable=False, default=0, comment="커뮤니티 ID")
    user_id = Column(Integer, nullable=False, default=0, comment="댓글 작성자 직원 ID")
    parent_id = Column(Integer, ForeignKey("communities_comments.id"), nullable=True, comment="부모 댓글 ID (대댓글용)")
