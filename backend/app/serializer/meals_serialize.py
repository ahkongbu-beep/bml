...
def get_feed_type_calendars_data(result):
    ...
    """
    try:
        def _format_input_date(v):
            input_date = getattr(v, 'input_date', None)
        ...
        return [
            MealsCalendarResponse(
                ...
                contents=v.contents,
                input_date=_format_input_date(v),
                month=v.month,
                child_id=v.child_id,
                ...
                is_liked=getattr(v, 'is_liked', False),
                ...
                refer_info={
                    ...
                    "refer_user_hash": getattr(v, 'refer_user_hash', None),
                    "refer_user_nickname": getattr(v, 'refer_user_nickname', None)
                } if getattr(v, 'refer_info', None) else None,
                mapped_tags=[
                    {
                    ...
                    for name, score, id in zip(
                        getattr(v, 'mapped_tags', '').split(',') if getattr(v, 'mapped_tags', None) else [],
                        getattr(v, 'mapped_scores', '').split(',') if getattr(v, 'mapped_scores', None) else [],
                        getattr(v, 'mapped_ids', '').split(',') if getattr(v, 'mapped_ids', None) else []
                    )
                ],
                user=FeedsUserResponse(
                    id=v.user_id,
                    nickname=getattr(v, 'nickname', None),
                    profile_image=getattr(v, 'profile_image', None),
                    user_hash=getattr(v, 'user_hash', None)
                ),
                childs=UserChildItemSchema(
                    child_id=v.agent_child_id if hasattr(v, "agent_child_id") and v.agent_child_id else v.child_id,
                    child_name=getattr(v, 'child_name', None),
                    child_birth=getattr(v, 'child_birth', None),
                    child_gender=getattr(v, 'child_gender', None),
                    is_agent=getattr(v, 'is_agent', None),
                    allergies=[
                        AllergyItemSchema(
                        ...
                        for code, name in zip(
                            getattr(v, 'allergy_codes', '').split(',') if getattr(v, 'allergy_codes', None) else [],
                            getattr(v, 'allergy_names', '').split(',') if getattr(v, 'allergy_names', None) else []
                        )
                    ] if getattr(v, 'allergy_names', None) else []
                ),
...
def feed_detail_response(meal_calendar, user, category, tags, images, child, allergies, comments, viewer_hash):
    ...
    return FeedsResponse(
        ...
        view_hash=meal_calendar.view_hash,
