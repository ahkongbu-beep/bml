...
def list_feeds(
    ...
    user_hash = getattr(request.state, "user_hash", None)
    return feeds_service.list_feeds(db, type=type, limit=limit, offset=offset, cursor=cursor, user_hash=user_hash, view_type=view_type, title=title, nickname=nickname, sort_by=sort_by, start_date=start_date, end_date=end_date, target_user_hash=target_user_hash)
