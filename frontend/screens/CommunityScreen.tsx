...
import { LoadingCommunityPage } from '@/components/LoadingCommunity';
import ConfirmPortal from '@/components/ConfirmPortal';

type CommunityListItem = CommunityPost | CommunityAdItem;

const getCommunityListItemKey = (item: CommunityListItem): string => {
  const typePrefix = item.is_ad ? 'ad' : 'community';
  return `${typePrefix}-${item.view_hash || item.id}`;
};
...
export default function CommunityScreen({ navigation }: any) {
        ...
        if (response.success && response.data) {
          ...
          setPosts((prevPosts) => {
            ...
            return mergedPosts.filter((post) => {
              const key = getCommunityListItemKey(post);
              if (seenKeys.has(key)) {
                return false;
              }
              seenKeys.add(key);
  ...
  return (
    <Layout>
      ...
      <View style={styles.tabContainer}>
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={[
              styles.tabButton,
              selectedCategory === "9999" && styles.tabButtonActive,
            ]}
            onPress={() => setSelectedCategory("9999")}
          >
            <Text
              style={[
                styles.tabButtonText,
                selectedCategory === "9999" && styles.tabButtonTextActive,
              ]}
            >
              베스트
            </Text>
      ...
      {/* 게시글 목록 */}
      {isLoading ? (
        <LoadingCommunityPage title="커뮤니티 정보를 불러오는 중"/>
      ) : (
        <FlatList
          data={posts}
          renderItem={renderPost}
          keyExtractor={(item) => getCommunityListItemKey(item)}
