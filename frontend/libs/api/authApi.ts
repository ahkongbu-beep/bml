...
export const register = async (data: RegisterRequest): Promise<ApiResponse<User>> => {
  return fetchPost<ApiResponse<User>>('/auth/register', data);
};
...
export const logout = async (): Promise<ApiResponse<null>> => {
  return fetchPost<ApiResponse<null>>('/auth/logout');
};
...
export const getMyInfo = async (): Promise<User> => {
  const response = await fetchPost<ApiResponse<User>>('/auth/me');
